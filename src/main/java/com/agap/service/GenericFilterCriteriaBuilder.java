...
public class GenericFilterCriteriaBuilder {
    ...
    private static final Map<String, Function<FilterCondition, Criteria>> FILTER_CRITERIA = new HashMap<String, Function<FilterCondition, Criteria>>();
    ...
    public Query addCondition(List<FilterCondition> andConditions, List<FilterCondition> orConditions) {
        ...
        List<Criteria> criteriaAndClause = buildCriteriaAndClause(filterAndConditions);

        List<Criteria> criteriaOrClause = buildCriteriaAndClause(filterOrConditions);
    ...
    List<Criteria> buildCriteriaAndClause(List<FilterCondition> filterWithConditions) {
        List<Criteria> criteriaWithClause = new ArrayList<>();
        filterWithConditions.stream().map(condition -> criteriaWithClause.add(buildCriteria(condition))).collect(Collectors.toList());
        return criteriaWithClause;
    }
    ...
    Criteria buildCriteria(FilterCondition condition) {
